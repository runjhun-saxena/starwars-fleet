...
export const useStarships = (params: UseStarshipsParams) => {
  return useInfiniteQuery({
    queryKey: ['starships', params.search],
    queryFn: ({ pageParam = 1 }): Promise<StarshipsResponse> =>
    ...
    select: (data) => {
      ...
      const pages = data.pages.map((p) => {
        ...
        if (hyper !== 'all') {
          results = results.filter(s => getHyperdriveCategory(s.hyperdrive_rating) === hyper);
        }
        if (crew !== 'all') {
          results = results.filter(s => getCrewCategory(s.crew) === crew);
        }
        return { ...p, results };
      });
